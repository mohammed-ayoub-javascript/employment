...
const NewProductForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const imageUrls = await Promise.all(
        imagesBase64.map(async (base64Image) => {
          try {
  ...
  return (
    <div className="w-full h-full flex flex-col p-5">
      <h1 className="text-4xl font-extrabold mb-4">نشر منتج جديد</h1>
      ...
      <p className="mt-4 mb-1">وصف قصير للمنتج</p>
      <Input
        name="description"
      ...
      <p className="mt-4 mb-1">عدد القطع المتاحة في المخزون</p>
      <Input
        name="stock"
        value={formData.stock}
        onChange={handleInputChange}
        placeholder="عدد القطع المتاحة في المخزون"
        type="number"
      />

      <p className="mt-4 mb-1">قسم المنتج</p>
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        {imagePreviews.map((src, index) => (
          <div key={index} className="relative P-3">
            <img src={src} alt="معاينة" className="w-[500px] h-[500px] object-cover rounded-md" />
            <button
